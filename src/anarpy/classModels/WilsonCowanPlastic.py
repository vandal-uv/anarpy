...
class WilsonCowanPlastic(WilsonCowan):
    ...
    def wilsonCowan(t, X, CM, sqdtD, N, G, P, tauE, tauI, a_ee, a_ie, a_ei, a_ii, rE, rI, rhoE, tau_ip, mu, sigma):
        E, I, a_ie = X
    ...
    def wilsonCowanCall(self, t, X):
        return self.wilsonCowan(t, X, self.CM, self.sqdtD, self.N, self.G, self.P, self.tauE, self.tauI, 
                                self.a_ee, self.a_ie, self.a_ei, self.a_ii, self.rE, self.rI, self.rhoE, self.tau_ip, self.mu, self.sigma)
    ...
    def wilsonCowanDet(t, X, CM, G, a_ee, a_ie, a_ei, a_ii, tauE, tauI, P, rE, rI, rhoE, tau_ip, mu, sigma):
        E, I, a_ie = X
    ...
    def sim_adapt(self, Init=None):
        ...
        """
        if Init is None:
            Var = np.array([self.E0, self.I0, self.a_ie_0])[:, None] * np.ones((1,self.N))
        else:
            Var = Init
...
if __name__=="__main__":
    ...
    Init = np.array([wc.E0,wc.I0,wc.a_ie_0])[:,None]*np.ones((1,wc.N))
