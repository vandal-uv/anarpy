...
Vr = -70  # V resting
DT = 2    # ΔT
Vth = -50  # Vthreshold of EIF model
Vreset = -58   # V reset (after spike)
Vspk = 0    # V for spike detection
gL = 6    # Leak conductance
C = 200    # Membrane capacitance
tau_exc = 5; Eexc = 0  # Tau and reversal V for Exc synapse
tau_inh = 20; Einh = -70   # Tau and reversal V for Inh synapse
...
N_i = [16, 10]  # Number of neurons per neuron type (exc, inh)
N = sum(N_i)   # Total number

# individual parameter values - see docstring in InitPars() function
# one value = all neurons the same
Vr_i = [-65, -62]  # -70
DT_i = [0.8, 3]  # 2
Vth_i = [-52, -42]  # -50
Vreset_i = [-53, -54]  # -58
...
def InitPars():
    ...
    from arrays of length K (number of neuron types).
    If any of the given values is a scalar, it will remain a scalar (i.e.,
    same value for all neurons).

    If the variable_i has length equal to the number of neurons, it will be copied
    as is (each value to each neuron).
    ...
    **This function does not take any argument**;
    ...
    These variables must exist either as a scalar or as a list of length K
    ...
    """
    global Vr, DT, Vth, Vreset, Vspk, gL, C, Ioffset, N
    N = sum(N_i)
    
    for param in Params:
        if type(eval(param+"_i")) == list:
            if eval("len("+param+"_i)") == len(N_i):
                exec(f"{param} = np.array([{param}_i[0],]*N_i[0] + [{param}_i[1],]*N_i[1])", globals())
            elif eval("len("+param+"_i)") == N:
                exec(f"{param} = np.array({param}_i)", globals())
            else:
                raise ValueError("check the length of variable" + param)
        else:
            exec(f"{param} = {param}_i", globals())
...
def runSim(X=None, recordV=False):
    ...
    if recordV:
        ...
        for i, t in enumerate(time):
            Istim = Ioffset+sqdt*np.random.normal(size=N)
...
def ParamsNet():
    pardict = {}
    for var in ('N_i', 'N', 'tau_exc', 'Eexc', 'tau_inh',
                'Einh', 'WsynE', 'WsynI', 'noise'):
        pardict[var] = eval(var)
    return pardict
